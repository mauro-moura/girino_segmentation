...
def load_images_return_shape(img_list, size_img = 160, new_size = None): # Anderson 2021.05.01
    '''
    Recebe um glob das imagens e converte em um numpy array no formato que o Keras aceita
    '''
    img = []
...
def load_images_return_shape_contr_stre(img_list, size_img = 160, new_size = None): # Anderson 2021.05.01
    '''
    Recebe um glob das imagens e converte em um numpy array no formato que o Keras aceita
    '''
    img = []
...
def load_images_return_shape_contr_stre2(img_list): # Anderson 2021.05.01
    '''
    Recebe um glob das imagens e converte em um numpy array no formato que o Keras aceita
    '''
    img = []
...
def resize_one_img(img, width, height): # Anderson 2021.04.30
    curr_img = cv2.resize(img, (width, height))
    #curr_img = np.reshape(curr_img, (width, height, 1))
    return curr_img
...
def normalize_2(images): # Para o caso de imagens muito escuras - Contrast Stretching Anderson 2021.05.01
    ...
    images = exposure.rescale_intensity(images, in_range=(p5,p95))
    
    # Depois
    #io.imshow(images_2[0])
    #plt.show()
