...
for i in range(n_fold):
    ...
    model1 = keras.models.load_model(folder_name + 's1/' + '%s.h5'%(filename[index]), compile=False)
    model2 = keras.models.load_model(folder_name + 's2/' + '%s.h5'%(filename[index]), compile=False)

    s1_pred = model1.predict(s1)
    s1_pred = resize_img(s1_pred, int(NEW_SIZE/2 - offset), NEW_SIZE)
    s1_pred = rebuild_parts(s1_pred, s1_ld, s1_lu, total_imgs)

    s2_pred = model2.predict(s2)
    s2_pred = resize_img(s2_pred, int(NEW_SIZE/2 - offset), NEW_SIZE)
    s2_pred = rebuild_parts(s2_pred, s2_ld, s2_lu, total_imgs)

    new_predicao = np.concatenate((s1_pred, s2_pred), axis = -1)
