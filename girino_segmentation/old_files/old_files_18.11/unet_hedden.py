...
def unet_hedden(size_img, SEED = 1):
    ...
    drop2 = Dropout(0.5, seed = SEED)(pool2)

    conv3 = Conv2D(96, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(drop2)
    conv3 = Conv2D(96, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    drop3 = Dropout(0.5, seed = SEED)(pool3)

    conv4 = Conv2D(192, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(drop3)
    conv4 = Conv2D(192, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    drop4 = Dropout(0.5, seed = SEED)(pool4)

    conv5 = Conv2D(384, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(drop4)
    conv5 = Conv2D(384, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(conv5)
    pool5 = MaxPooling2D(pool_size=(2, 2))(conv5)
    drop5 = Dropout(0.5, seed = SEED)(pool5)

    conv6 = Conv2D(768, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(drop5)
    conv6 = Conv2D(768, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(conv6)
    pool6 = MaxPooling2D(pool_size=(2, 2))(conv6)
    drop6 = Dropout(0.5, seed = SEED)(pool6)

    # Camada do Meio
    conv7 = Conv2D(1536, 3, activation = 'relu', padding = 'same', kernel_initializer = INITIALIZER)(drop6)
