...
from utils import create_folder, load_images_array, resize_img
...
def concat_parts(data, s1_ld, s1_lu, s2_ld, s2_lu, offset = 0):
    s1, s2 = cut_img_list(data, offset)
    ...
    conc = np.concatenate((s1, s2), axis = 0)
...
X = concat_parts(X, s1_ld, s1_lu, s2_ld, s2_lu, offset)
Y = concat_parts(Y, s1_ld, s1_lu, s2_ld, s2_lu, offset)
print(X.shape)
