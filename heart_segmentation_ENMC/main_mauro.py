...
def mean_filter(image, filter_size):
    ...
    return new_image
def apply_mean_filter(imgs_load):
    filter_size = 9 
    imgs_list = []
...
dice_metric = []
tempo = []
local = 'heart_tests/heart_LOO_DA/'
...
for i in range(len(img_xtrain)):
    ...
    #train_X = apply_mean_filter(train_X)
    #valid_X = apply_mean_filter(valid_X)
    #train_ground = apply_mean_filter(train_ground)
    #valid_ground = apply_mean_filter(valid_ground)
    ...
    valid_generator = image_datagen.flow(valid_X, valid_ground,
            batch_size=use_batch_size,
            seed=SEED_2)
    ...
    reduce = ReduceLROnPlateau(monitor = 'val_loss', patience = 2, verbose = 1)
    ...
    #history = model.fit(x = train_X, y = train_ground, batch_size = 4, epochs = 100, callbacks = callback)
    final = time()
